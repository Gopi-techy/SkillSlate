...
export class CreatePortfolio {
  ...
  render() {
    console.log('CreatePortfolio render called for user:', this.user?.email);
    ...
    try {
      ...
      console.log('CreatePortfolio content generated, length:', content.length);
      return content;
    } catch (error) {
      console.error('CreatePortfolio render error:', error);
      return `
        <div class="min-h-screen py-8">
          <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 class="text-4xl font-bold text-white mb-2">Create Portfolio Error</h1>
            <p class="text-red-400">Error rendering create portfolio: ${error.message}</p>
          </div>
        </div>
      `;
    }
  ...
  attachEventListeners() {
    console.log('CreatePortfolio attachEventListeners called for step:', this.step);
    
    // Input method selection
    document.querySelectorAll('[data-input-method]').forEach(button => {
      button.addEventListener('click', (e) => {
        e.preventDefault();
        const method = button.getAttribute('data-input-method');
        if (method) {
    ...
    navButtons.forEach((button, index) => {
      ...
      console.log(`Navigation button ${index}:`, page, button.tagName, button.textContent?.trim());
      ...
      button.addEventListener('click', (e) => {
        ...
        if (this.isGenerating || this.isDeploying) {
          console.log('Navigation blocked - operation in progress');
          return;
        }
        
        const page = button.getAttribute('data-nav');
        console.log('CreatePortfolio navigation clicked:', page);
        if (page && this.onNavigate) {
          this.onNavigate(page);
        } else {
          console.warn('No page or onNavigate function available:', { page, onNavigate: !!this.onNavigate });
        }
      });
    });
  }
